...
ble.onRead = function (data, uuid){
	...
	//console.log(array_atai[0]);
	
	for(i=0;i<3;i++){
		data1=Number(array_atai[i])/4095;
		data1=data1*5000;
		data1=Math.round(data1);
		M5sen1_val[i] = Number(data1);
	}
	
	for(i=0;i<3;i++){
		M5sen1_val2[i] = M5sen1_val[i].toString().padStart(4,'0');
	}
	
	//HTMLにデータを表示
	for(i=0;i<3;i++){
		document.getElementById(data_display[i]).innerHTML = M5sen1_val2[i];
	}
...
ble3.onRead = function (data, uuid){
	...
	for(i=0;i<3;i++){
		...
		data1=data1*5000;
		data1=Math.round(data1);
		M5sen2_val[i] = Number(data1);
	}
	
	for(i=0;i<3;i++){
		M5sen2_val2[i] = M5sen2_val[i].toString().padStart(4,'0');
	}
	
	//HTMLにデータを表示
	for(i=3;i<6;i++){
		document.getElementById(data_display[i]).innerHTML = M5sen2_val2[t];
		t+=1;
	}
...
function Create_grapf() {
	...
	if(startflag1==1 &&  startflag2==1){
		...
		for(i=0;i<3;i++){
			array1[99][i]=M5sen1_val[i];
		}
		
		for(i=3;i<6;i++){
			array1[99][i]=M5sen2_val[t];
			t+=1;
		}
